...
def load_playlist():
  ...
  # unencode
  playlist_url = urllib2.unquote(playlist_url)

  api = get_rdio_api()
  result = api.call('getObjectFromUrl', {
    'url': playlist_url,
    'extras': 'tracks',
  })
...
@app.route('/loadMoreSongs')
def load_more_songs():
  params = dict([part.split('=') for part in request.query_string.split('&')])

  # it looks like -Track.*,Track.key isn't supported by rdio yet
  api = get_rdio_api()
  result = api.call('get', {
    'keys': params['key'],
    'extras': 'tracks,-Track.*,Track.key',
  })
